...
def update_swa(swa_model, model, alpha):
    for swa_param, param in zip(swa_model.parameters(), model.parameters()):
        swa_param.data = alpha * swa_param.data + (1.0 - alpha) * param.data


def run_experiment(dataset: str, method: str, seed: int, device: torch.device, args, hparams_override: dict = None) -> dict:
    set_seed(seed)
    ...
    if method == "swad":
        ...
        swa_start = int(epochs * 0.75) # Start SWA at 75% of epochs
        swa_n = 0
    ...
    if hparams_override:
        method_hparams.update(hparams_override)
    ...
    for epoch in range(epochs):
        ...
        if method == "swad" and epoch >= swa_start:
            update_swa(swa_model, model, swa_n / (swa_n + 1))
            swa_n += 1

    eval_model = swa_model if method == "swad" else model
    if method == "swad":
        # BatchNorm running stats update is skipped for MLP, but for CNNs it would be needed.
        pass

    env_accs, env_losses, mean_acc, worst_acc = evaluate(eval_model, test_loaders, criterion, device)
    return {
        "dataset": dataset,
        "method": method,
        "seed": seed,
...
def parse_args():
    ...
    parser.add_argument("--results-path", default=os.path.join("exp", "results", "results_v2.csv"))
...
def main():
    ...
    meta_path = os.path.join(os.path.dirname(args.results_path), "run_meta_v2.json")
